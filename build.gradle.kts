...
plugins {
    ...
    // Coverage is reported, never a merge gate: adr/0021. The canonical gate list is
    // test.definition.md § 7, and a threshold there would be defended by testing what is
    // easy rather than what is risky.
    jacoco
}
...
dependencies {
    ...
    // Operational baseline, adr/0021: the platform collects both OTLP and Prometheus.
...
tasks.named<Test>("test") {
    ...
    // Three of these tests read the *documentation* rather than the code —
    // `ContextRegistryTest`, `SpecCitationsTest` and `DocumentationLinksTest`. Gradle has no
    // way to know that, so a documentation-only change left `test` UP-TO-DATE and the gates
    // that exist to catch documentation rot did not run at all. Declaring the folders as
    // inputs is what makes them fire on the commits they were written for.
    inputs.dir(layout.projectDirectory.dir("documentation")).withPropertyName("documentation")
    inputs.dir(layout.projectDirectory.dir("api")).withPropertyName("api")
}
...
// Coverage report, adr/0021. Deliberately **not** wired into `check`: the canonical merge
// gate list is `test.definition.md` § 7 and a coverage threshold is absent from it. Run
// `./gradlew test jacocoTestReport` to produce it.
